...
public class SecurityUser implements UserDetails { // phân quyền người dùng
  ...
  @JsonIgnore private String password; // password người dùng không được trả về json khi response cho client
  ...
  public static SecurityUser build(User user) { // hàm tạo 1 SecurityUser từ User
    ...
    if (user.isAdmin()) authorities.add(buildRole(UserRole.ADMIN.name())); // nếu là admin thì thêm quyền admin vào 
    else if (user.isMember()) authorities.add(buildRole(UserRole.MEMBER.name())); // nếu là member thì thêm quyền member vào

    return new SecurityUser(user.getId(), user.getPassword(), authorities, user); // dùng hàm tạo full đối tượng tạo 1 SecurityUser
  }

  private static GrantedAuthority buildRole(String role) { // hàm tạo 1 quyền từ String role
    return new SimpleGrantedAuthority(String.format(ROLE_PREFIX, role)); // tạo mới 1 đơn quyền từ String 
  }

  @Override
  public Collection<? extends GrantedAuthority> getAuthorities() { // lấy tập hợp các role của người dùng
