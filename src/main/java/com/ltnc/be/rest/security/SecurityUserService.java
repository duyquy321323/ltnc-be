...
@RequiredArgsConstructor // hàm contructor cho các trường bắt buộc chưa những từ khóa như là final hoặc có annotation @NonNull 
public class SecurityUserService implements UserDetailsService {
  ...
  public static User getCurrentUser() {
    var principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal(); // trả về người dùng hiện tại dạng UserDetails
    if (ANONYMOUS_USER.equals(principal)) throw new PermissionDeniedException(); // nếu người dùng hiện tại là ẩn danh thì ném ngoại lệ
  ...
  @Override
  public UserDetails loadUserByUsername(String id) throws UsernameNotFoundException { // lấy userdetail ra từ csdl bằng id
    var user =
        this.userRepository.findById(Long.parseLong(id)).orElseThrow(() -> new UsernameNotFoundException(id + " is not found"));
